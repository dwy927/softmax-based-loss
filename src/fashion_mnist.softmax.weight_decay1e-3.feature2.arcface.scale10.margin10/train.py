...
def main():
    ...
    while True:
        ...
        for idx, data in enumerate(data_loader_train):
            ...
            optimizer.zero_grad()
            outputs_2 = arc_face(outputs, y_train, scale, margin)
            ...
            if idx % 100 == 0: # print train info
                logger.info("step: {}, train arcface loss: {:.4f}, ce loss: {:.4f}, train acc: {:.4f}".format(idx, loss_amsoftmax.item(), loss_ce.item(), step_correct / len(y_train)))
            clock.tick()
